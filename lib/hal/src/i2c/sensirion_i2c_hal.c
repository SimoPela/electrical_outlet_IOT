...
int16_t sensirion_i2c_hal_read(uint8_t address, uint8_t* data, uint16_t count) {
    return i2c_master_read_from_device(I2C_PORT, address, data, count, pdMS_TO_TICKS(1000));
}
...
int16_t sensirion_i2c_hal_write(uint8_t address, const uint8_t* data, uint16_t count) {
    return i2c_master_write_to_device(I2C_PORT, address, data, count, pdMS_TO_TICKS(1000));
}
