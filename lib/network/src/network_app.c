...
/**
 * @file network_app.c
 * @brief Wi-Fi station bring-up and connection gating implementation.
 *
 * Uses the ESP-IDF event loop: @c WIFI_EVENT and @c IP_EVENT are handled by a
 * single static handler.  The event group bits @c APP_WIFI_CONNECTED_BIT and
 * @c APP_WIFI_FAIL_BIT are used by @ref network_app_wait_until_connected to
 * gate MQTT startup in @c app_main.
 */
...
/**
 * @brief Unified Wi-Fi and IP event handler.
 *
 * Handles:
 *  - @c WIFI_EVENT_STA_START     — initiates connection.
 *  - @c WIFI_EVENT_STA_DISCONNECTED — retries up to @c APP_WIFI_MAX_RETRY times,
 *    then sets @c APP_WIFI_FAIL_BIT.
 *  - @c IP_EVENT_STA_GOT_IP — sets @c APP_WIFI_CONNECTED_BIT and updates
 *    @c g_device_state.wifi_connected under the device-state mutex.
 *
 * @param arg        Unused.
 * @param event_base @c WIFI_EVENT or @c IP_EVENT.
 * @param event_id   Event identifier within @p event_base.
 * @param event_data Event-specific data; cast depends on @p event_id.
 */
...
/**
 * @brief Initialize NVS flash, erasing and reinitialising if the partition is stale.
 *
 * @return @c true on success, @c false on fatal NVS error.
 */
...
/** @copydoc network_app_wait_until_connected */
